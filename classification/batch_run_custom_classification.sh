...
export BERT_MODEL=microsoft/deberta-v3-base  # microsoft/mdeberta-v3-base, microsoft/deberta-v3-base, bert-base-multilingual-cased
export MODEL_TYPE=AutoTuneFaissKNNDeBERTaV3ForSequenceClassification
export BATCH_SIZE=32
export DATA_NAME=mnli    # sentiment_classification: imdb, yelp_polarity nli: mnli, snli
export USE_OOD_TEST=true  # if true, inference for OOD datasets.
export DATA_PATH=$BASE_DATA_DIR/$DATA_NAME

export TAU=1.0
export SMOOTHING=0.1   # For LS. [0.01, 0.05, 0.1, 0.2, 0.3]
export STOPPING_CRITERIA=accuracy_score  # f1, ece, loss, accuracy_score
export OPTIMIZE_TYPE=maximize
export NUM_EPOCHS=200   # default: 200
...
export PN_LATENT_DIM=10

# for kNN's
export DENSITY_ESTIMATOR_PATH=models/mnli-BaselineDeBERTaV3ForSequenceClassification-RealNVP-density-estimation-accuracy_score
export DENSITY_ESTIMATOR_TYPE=RealNVP
export KNN_TOPK=32    # 8, 16, 32, 64, 128
export N_LIST=100  
export N_SUBVECTORS=32
export KNN_NPROBE=64
export INDEX_TYPE=FLAT
export KNN_ALPHA=1.0
export KNN_TEMPERATURE=800
export USE_NEIGHBOR_LABELS=true
...
if [[ "$MODEL_TYPE" == "FaissKNNDeBERTaV3ForSequenceClassification" ]] ; then
    export BERT_MODEL=models/imdb-imdb-BaselineDeBERTaV3ForSequenceClassification-accuracy_score-saved
fi
...
if  [[ "$MODEL_TYPE" == *"KNNDeBERTaV3ForSequenceClassification"* ]] ; then
...
if  [[ "$MODEL_TYPE" == "AutoTuneDACDeBERTaV3ForSequenceClassification" ]] ; then
    OUTPUT_DIR+="-K$KNN_TOPK-NPROBE$KNN_NPROBE-$INDEX_TYPE"
