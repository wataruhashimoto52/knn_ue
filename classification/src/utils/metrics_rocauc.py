...
def compute_roc_auc_ner_flatten(
...
def compute_roc_auc_ner_flatten_without_other(
    evalprediction: EvalPredictionV2,
    label_map: dict,
    ignore_index: Optional[int] = None,
    average: str = "macro",
    ...
    score = multiclass_auroc(
        preds=torch.from_numpy(np.asarray(without_o_predictions)),
        target=torch.from_numpy(np.asarray(without_o_labels)),
        num_classes=confidences.shape[-1],
        average=average,
        ignore_index=ignore_index,
    )
...
def compute_roc_auc_ner_flatten_span(
    evalprediction: EvalPredictionV2,
    label_map: dict,
    ignore_index: Optional[int] = None,
    use_mean: bool = False,
...
def compute_roc_auc_ner_sentence(
    evalprediction: EvalPredictionV2,
    label_map: dict,
    ignore_index: Optional[int] = None,
    average: str = "macro",
    ...
    for i in range(len(confidences)):
        ...
        if evalprediction.tags:  # with CRF
            final_confidences = np.asarray(
                [p for (p, a) in zip(confidence, attentions[i]) if int(a) == 1]
            )
        else:
            final_confidences = np.asarray(
        ...
        if len(set(final_labels.tolist())) == 1 and final_labels[0] == o_tag:
            continue
...
def compute_roc_auc_ner_sentence_without_other(
    evalprediction: EvalPredictionV2,
    label_map: dict,
    ignore_index: Optional[int] = None,
    average: str = "macro",
...
def compute_roc_auc_ner_sentence_span(
    evalprediction: EvalPredictionV2,
    label_map: dict,
    ignore_index: Optional[int] = None,
    use_mean: bool = False,
    ...
    for i in range(len(confidences)):
        ...
        if evalprediction.tags:  # with CRF
            processed_confidences = np.asarray(
                [p for (p, a) in zip(confidences[i], attentions[i]) if int(a) == 1]
            )
        else:
            processed_confidences = np.asarray(
