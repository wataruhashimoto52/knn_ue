...
def get_tagger_one_hot(label, N, off_value, on_value, input_mask, gpu, is_2d):
    ...
    if is_2d:
        ones = rearrange(ones, "b l k -> (b l) k", b=batch_size)
...
class Tagger_Evidence(nn.Module):
    ...
    def __init__(
        ...
        use_span_weight: bool = False,
        total_epoch: int = 30,
        with_kldiv: bool = True,
        with_iw: bool = False,
        with_uc: bool = True,
    ):
        super().__init__()
        ...
        self.etrans_func = etrans_func
        self.loss_type = loss_type
        self.annealing_start =annealing_start
        self.annealing_step = annealing_step
        self.use_span_weight = use_span_weight
        self.loss_func = CrossEntropyLoss()
        self.total_epoch = total_epoch
        self.eps = 1e-10
        self.with_kldiv = with_kldiv
        self.with_iw = with_iw
        self.with_uc = with_uc
    ...
    def mes_loss(self, logits, loadall, sequence_labels, input_mask, epoch):
        ...
        E = alpha - 1
        m = alpha / S
        soft_output = get_tagger_one_hot(sequence_labels, self.num_classes, self.device_type)
    ...
    def edl_loss(self, logits, sequence_labels, input_mask, epoch):
        ...
        soft_output = get_tagger_one_hot(sequence_labels, self.num_classes, 0, 1, input_mask, self.device_type) # one-hot label
        alpha = evidence_trans(logits, self.etrans_func) + 1
        evidence = alpha -1
        S = torch.sum(alpha, dim=2, keepdim=True)  # Dirichlet strength
        bu = evidence/S
