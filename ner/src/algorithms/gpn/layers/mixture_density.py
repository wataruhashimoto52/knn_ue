...
class MixtureDensity(nn.Module):
    ...
    def __init__(
        self, dim: int, n_components: int = 20, mixture_type: str = "normal_mixture"
    ):
        super().__init__()
        self.dim = dim
        ...
        if self.mixture_type == "normal_mixture":
            ...
            self.log_sigma_ = torch.nn.Parameter(
                torch.randn(self.n_components, self.dim, self.dim) / 100.0
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        sigma = (
            sigma
            - torch.diag_embed(torch.diagonal(sigma, dim1=-2, dim2=-1))
            + torch.diag_embed(torch.diagonal(torch.exp(sigma), dim1=-2, dim2=-1))
            + 0.001
            * torch.diag_embed(torch.ones(self.n_components, self.dim)).to(
                x.device.type
            )
        )
