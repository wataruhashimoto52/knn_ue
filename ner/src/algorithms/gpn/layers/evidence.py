...
# from gpn.distributions import ConjugatePrior, Likelihood
from torch import Tensor

from .mixture_density import MixtureDensity
from .normalizing_flow import BatchedNormalizingFlowDensity
from ...density_estimators import NormalizingFlow
...
class Density(nn.Module):
    ...
    def forward(self, z: Tensor) -> Tensor:
        ...
        if self.use_batched_flow:
            ...
            log_q_c = self.forward_flow_custom(z)
    ...
    def forward_flow_custom(self, z: Tensor) -> Tensor:
        n_nodes = z.size(0)
        log_q = torch.zeros((n_nodes, self.num_mixture_elements)).to(z.device.type)
